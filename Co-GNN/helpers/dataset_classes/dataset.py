...
def load_graphsat_dataset(file_path):
    ...
    i = 0
    while i < len(lines):
        num_nodes, label = map(int, lines[i].strip().split())
        i += 1
        adj_matrix = np.zeros((num_nodes, num_nodes), dtype=np.float32)
        for _ in range(num_nodes):
            node_idx, *neighbors = map(int, lines[i].strip().split())
            for neighbor in neighbors:
                adj_matrix[node_idx - 1, neighbor - 1] = 1  # Creating adjacency matrix
            i += 1
...
# Replace with the actual path to your GRAPHSAT.txt file
graphs, labels = load_graphsat_dataset('helpers/dataset_classes/GRAPHSAT.txt')

def create_pyg_data(graphs, labels):
    pyg_data_list = []
    for adj_matrix, label in zip(graphs, labels):
        # Find the indices of the edges
        edge_index = torch.tensor(np.array(np.nonzero(adj_matrix)), dtype=torch.long)
        # Create dummy node features (if actual features are not available)
        x = torch.ones((adj_matrix.shape[0], 1), dtype=torch.float)
...
# Assuming 'graphs' and 'labels' are obtained from the load_graphsat_dataset function
exp = create_pyg_data(graphs, labels)
...
class DataSet(Enum):
    ...
    def get_family(self) -> DataSetFamily:
        ...
        elif self in [DataSet.root_neighbours, DataSet.cycles, DataSet.exp]:
    ...
    def is_expressivity(self) -> bool:
        return self is DataSet.cycles or (self is DataSet.exp)
    ...
    def get_metric_type(self) -> MetricType:
        if self.get_family() in [DataSetFamily.social_networks, DataSetFamily.proteins, DataSetFamily.homophilic]\
                or self in [DataSet.roman_empire, DataSet.amazon_ratings, DataSet.cycles, DataSet.exp]:
